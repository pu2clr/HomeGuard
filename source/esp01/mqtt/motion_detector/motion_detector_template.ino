...
const long gmtOffset_sec = -3 * 3600;  // UTC-3 (Brasil/Brasília)
...
void initializeNTP() {
  ...
  int attempts = 0;
  while (nowSecs < 8 * 3600 * 2 && attempts < 10) {
    delay(500);
    Serial.print(".");
...
// ======== Get Current Timestamp (Brasil/Brasília UTC-3) ========
...
void publishHeartbeat() {
  ...
  heartbeat += "\"unix_timestamp\":\"" + String(getUnixTimestamp()) + "\",";
...
void checkMotionSensor() {
  ...
  if (currentMotionReading && !lastMotionState && 
    ...
    motionEvent += "\"timestamp\":\"" + getCurrentTimestamp() + "\",";
    motionEvent += "\"unix_timestamp\":\"" + String(getUnixTimestamp()) + "\",";
