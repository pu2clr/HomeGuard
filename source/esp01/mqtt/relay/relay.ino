...
#define RELAY_ACTIVE_LOW                                                       \
  true // Changed: Your relay activates with HIGH, so it's ACTIVE HIGH
...
String TOPIC_COMMAND =
    "home/relay/" + String(DEVICE_ID) + "/command"; // Receive commands
String TOPIC_STATUS =
    "home/relay/" + String(DEVICE_ID) + "/status"; // Send status
String TOPIC_INFO =
    "home/relay/" + String(DEVICE_ID) + "/info"; // Send device info
...
const unsigned long STATUS_SEND_INTERVAL =
    5000; // Send status every 5 seconds if changed
...
void sendStatus() {
  ...
  client.publish(TOPIC_STATUS.c_str(), status.c_str(),
...
void setup() {
  ...
  // Set initial relay state (OFF) - this won't send MQTT since
  // device_status.online = false
  setRelay(false);
