/**
  ...
  - Real-time timestamps via NTP synchronization
  - Configurable sensitivity and detection time
  - MQTT status reporting with LWT
  - Remote monitoring capabilities
  - Device identification and heartbeat
   } else if (command.equalsIgnoreCase("SYNC_NTP")) {
    DEBUG_PRINTLN("🔄 Forçando sincronização NTP...");
    ntpConfigured = false;
    configureNTP();
    
    // Send current time status
    time_t now = 0;
    time(&now);
    if (now >= 1609459200) {
      struct tm *timeinfo = localtime(&now);
      DEBUG_PRINTF("🕒 Hora atual após sync: %04d-%02d-%02d %02d:%02d:%02d\n", 
                  timeinfo->tm_year + 1900, timeinfo->tm_mon + 1, timeinfo->tm_mday,
                  timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec);
    } else {
      DEBUG_PRINTF("❌ NTP ainda falhou - timestamp: %u\n", (unsigned int)now);
    }
    
  } else if (command.equalsIgnoreCase("TEST_EVENT")) {
    DEBUG_PRINTLN("🧪 Enviando evento de teste...");
    publishEvent(true);
    delay(2000);
    publishEvent(false);
    
  } else {tamp formatting with date/time fields
  - Automatic NTP re-synchronization
  - Robust reconnection with backoff
  
  MQTT Commands:
  - mosquitto_sub -h 192.168.18.198 -u homeguard -P pu2clr123456 -t "home/motion/MOTION_01/#" -v
  - mosquitto_pub -h 192.168.18.198 -t home/motion/MOTION_01/command -m "STATUS" -u homeguard -P pu2clr123456
  - mosquitto_pub -h 192.168.18.198 -t home/motion/MOTION_01/command -m "ENABLE" -u homeguard -P pu2clr123456
  - mosquitto_pub -h 192.168.18.198 -t home/motion/MOTION_01/command -m "SYNC_NTP" -u homeguard -P pu2clr123456
  - mosquitto_pub -h 192.168.18.198 -t home/motion/MOTION_01/command -m "TEST_EVENT" -u homeguard -P pu2clr123456
*/
...
#define NTP_SERVER_3     "br.pool.ntp.org"
#define TIMEZONE_OFFSET  -3  // UTC-3 for Brazil (Brasília)
#define DST_OFFSET       0   // No DST adjustment
...
void configureNTP() {
  if (!WiFi.isConnected()) {
    DEBUG_PRINTLN("WiFi não conectado - pulando NTP");
    return;
  }
  
  DEBUG_PRINTLN("Configurando sincronização NTP...");
  
  // Configure timezone and NTP servers - usando servidores mais próximos
  configTime(-3 * 3600, 0, "pool.ntp.br", "a.ntp.br", "b.ntp.br");
  
  DEBUG_PRINTLN("Aguardando sincronização NTP...");
  
  // Wait for time synchronization with patience
  time_t now = 0;
  int attempts = 0;
  
  while (now < 1609459200 && attempts < 60) { // Unix timestamp for 2021-01-01
    time(&now);
    DEBUG_PRINTF("Tentativa %d - Timestamp: %u ", attempts + 1, (unsigned int)now);
  ...
  if (now >= 1609459200) {
    ...
    DEBUG_PRINTLN("✅ NTP SINCRONIZADO COM SUCESSO!");
    DEBUG_PRINTF("📅 Data/Hora: %04d-%02d-%02d %02d:%02d:%02d\n", 
                timeinfo->tm_year + 1900, timeinfo->tm_mon + 1, timeinfo->tm_mday,
                timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec);
  } else {
    ntpConfigured = false;
    DEBUG_PRINTF("❌ Falha na sincronização NTP após %d tentativas\n", attempts);
    DEBUG_PRINTLN("🔄 Usando timestamp de uptime como fallback");
  }
}
...
String getTimestamp() {
  ...
  DEBUG_PRINTF("🕒 getTimestamp() - now: %u, ntpConfigured: %s\n", 
               (unsigned int)now, ntpConfigured ? "SIM" : "NÃO");
  
  // Check if we have a valid timestamp (after 2021)
  if (now >= 1609459200) {  // 2021-01-01
    struct tm *timeinfo = localtime(&now);
    
    char timestamp[32];
    // Format: 2025-09-01T14:30:45-03:00
    snprintf(timestamp, sizeof(timestamp), "%04d-%02d-%02dT%02d:%02d:%02d-03:00",
             timeinfo->tm_year + 1900, timeinfo->tm_mon + 1, timeinfo->tm_mday,
             timeinfo->tm_hour, timeinfo->tm_min, timeinfo->tm_sec);
    
    ntpConfigured = true;
    DEBUG_PRINTF("✅ Timestamp real: %s\n", timestamp);
    return String(timestamp);
  } else {
    // Fallback to uptime
    unsigned long uptime = (millis() - bootTime) / 1000;
    ntpConfigured = false;
    String fallback = "uptime_" + String(uptime);
    DEBUG_PRINTF("🔄 Timestamp fallback: %s\n", fallback.c_str());
    return fallback;
  }
}
...
void syncNTPIfNeeded() {
  ...
  // If NTP is not configured, try more frequently (every 30 seconds initially)
  unsigned long syncInterval = ntpConfigured ? NTP_SYNC_INTERVAL : 30000; // 30 seconds if not working
  
  if (WiFi.isConnected() && (currentTime - lastNtpSync >= syncInterval)) {
    DEBUG_PRINTF("🔄 Tentando sincronizar NTP... (última tentativa há %lu ms)\n", 
                currentTime - lastNtpSync);
    configureNTP();
    lastNtpSync = currentTime;
  }
  
  // Also check if current time is valid periodically
  static unsigned long lastTimeCheck = 0;
  if (currentTime - lastTimeCheck >= 10000) { // Check every 10 seconds
    time_t now = 0;
    time(&now);
    
    if (!ntpConfigured && now >= 1609459200) {
      DEBUG_PRINTLN("🎉 NTP sincronizado automaticamente detectado!");
      ntpConfigured = true;
    }
    
    DEBUG_PRINTF("⏰ Status NTP: %s, Timestamp: %u\n", 
                ntpConfigured ? "ATIVO" : "INATIVO", (unsigned int)now);
...
bool connectWifi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Configure NTP after successful WiFi connection
    configureNTP();
...
void publishEvent(bool motion) {
  if (!client.connected() || !motionEnabled) {
    DEBUG_PRINTLN("❌ publishEvent: MQTT desconectado ou movimento desabilitado");
    return;
  }
  
  DEBUG_PRINTF("🚀 publishEvent: motion=%d\n", motion ? 1 : 0);
  
  // Get current timestamp
  String currentTimestamp = getTimestamp();
  
  // Create JSON payload with full datetime information
  String payload = "{";
  payload += "\"motion\":" + String(motion ? 1 : 0) + ",";
  payload += "\"ts\":\"" + currentTimestamp + "\",";
  
  // Add separate date and time fields if NTP is working
  time_t now = 0;
  time(&now);
  if (now >= 1609459200) { // Valid timestamp (after 2021)
    struct tm *timeinfo = localtime(&now);
    ...
    DEBUG_PRINTF("📅 NTP ativo - Data: %s, Hora: %s\n", dateStr, timeStr);
  } else {
    DEBUG_PRINTLN("🔄 NTP indisponível - usando timestamp de uptime");
  }
...
void publishHeartbeat() {
  ...
  payload += "\"ntp_synced\":" + String(ntpConfigured ? "true" : "false");
  ...
  client.publish(TOPIC_HEARTBEAT.c_str(), payload.c_str(), false); // not retained
