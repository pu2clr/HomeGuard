...
# Sensor configurations (Bash associative array)
declare -A SENSORS=(
...
check_libraries() {
    ...
    local required_libs=("PubSubClient" "ESP8266WiFi")
...
select_usb_port() {
    ...
    while true; do
        read -p "Enter the USB port (full path, e.g., /dev/tty.usbserial-XXX): " usb_port
...
show_sensor_menu() {
    ...
    for key in $(echo "${!SENSORS[@]}" | tr ' ' '\n' | sort -n); do
        IFS=',' read -r location ip topic <<< "${SENSORS[$key]}"
...
select_sensor() {
    while true; do
        show_sensor_menu
        read -p "Select sensor to compile and upload (1-6, q): " choice
        
        case $choice in
            [1-5])
                if [[ -n "${SENSORS[$choice]}" ]]; then
                    IFS=',' read -r selected_location selected_ip selected_topic <<< "${SENSORS[$choice]}"
            ...
            6)
                echo ""
                read -p "Enter location name (no spaces, use underscore): " selected_location
                read -p "Enter IP last octet (201-255): " selected_ip
                read -p "Enter MQTT topic suffix (e.g., motion_custom): " selected_topic
...
prepare_sketch() {
    ...
    echo "sketch_dir=$sensor_dir"  # Return directory path for arduino-cli
}
...
compile_sketch() {
    ...
    build_flags+="-DDEVICE_LOCATION=$selected_location "
    build_flags+="-DDEVICE_IP_LAST_OCTET=$selected_ip "
    build_flags+="-DMQTT_TOPIC_SUFFIX=$selected_topic"
    
    print_info "Build flags: $build_flags"
...
show_post_upload_instructions() {
    ...
    echo "   mosquitto_sub -h 192.168.18.198 -u homeguard -P pu2clr123456 -t \"home/$selected_topic/#\" -v"
    echo ""
    echo "   Check device status:"
    echo "   mosquitto_pub -h 192.168.18.198 -t \"home/$selected_topic/cmnd\" -m \"STATUS\" -u homeguard -P pu2clr123456"
    echo ""
...
open_serial_monitor() {
    ...
    read -p "Do you want to open serial monitor? (y/N): " open_monitor
...
main() {
    ...
    # Prepare sketch
    sketch_result=$(prepare_sketch)
    sketch_dir=$(echo "$sketch_result" | grep "sketch_dir=" | cut -d'=' -f2)
    ...
    if compile_sketch "$sketch_dir"; then
        ...
        read -p "Is the ESP-01S ready for upload? (y/N): " ready_upload
